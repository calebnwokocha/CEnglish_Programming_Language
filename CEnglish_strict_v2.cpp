...
namespace CEnglish {
    ...
    private:
        ...
        std::unordered_set<std::string> active_custom_tokens_;
        ...
        static const std::unordered_set<std::string>& reserved_user_words() {
            static const std::unordered_set<std::string> words = {
                "a", "an", "the", "this", "that", "these", "those", "my", "your", "our", "his", "her", "its", "their",
                "one", "all", "some", "any", "no", "most", "other", "only", "few", "many", "much",
                "of", "in", "on", "with", "by", "from", "at", "for", "as", "to", "into", "over", "after", "before",
                "because", "than", "through", "under", "between", "without", "within", "about",
                "be", "is", "am", "are", "was", "were", "been", "being", "have", "has", "had", "do", "does", "did",
                "shall", "should", "will", "would", "can", "could", "may", "might", "must",
                "here", "there", "where", "when", "who", "whom", "whose", "what", "which", "why", "how",
        ...
        static const std::unordered_set<std::string>& phrase_boundary_words() {
            static const std::unordered_set<std::string> words = {
                "a", "an", "the", "this", "that", "these", "those", "my", "your", "our", "his", "her", "its", "their",
                "of", "in", "on", "with", "by", "from", "at", "for", "as", "to", "into", "over", "after", "before",
                "because", "than", "through", "under", "between", "without", "within", "about",
        ...
        static const std::unordered_set<std::string>& determiner_words() {
            static const std::unordered_set<std::string> words = {
                "a", "an", "the", "this", "that", "these", "those", "my", "your", "our", "his", "her", "its", "their",
                "one", "all", "some", "any", "no", "most", "other", "only", "few", "many", "much"
            };
        ...
        static const std::unordered_set<std::string>& relation_words() {
            static const std::unordered_set<std::string> words = {
                "is", "be", "have", "of", "in", "on", "with", "by", "from", "at", "for", "as", "to", "into", "over", "after", "because", "than", "way", "then"
            };
            return words;
        }

        bool is_reserved_word(const std::string& name) const {
            return reserved_user_words().count(name) != 0;
        }

        bool is_phrase_boundary_word(const std::string& name) const {
            return phrase_boundary_words().count(name) != 0;
        }

        bool is_determiner_word(const std::string& name) const {
            return determiner_words().count(name) != 0;
        }

        bool is_relation_word(const std::string& name) const {
            return relation_words().count(name) != 0;
        }

        bool definition_is_recursive(const CustomToken& tok) const {
            return std::find(tok.definition_tokens.begin(), tok.definition_tokens.end(), tok.name) != tok.definition_tokens.end();
        }
        ...
        TokenSpec strict_seed_spec(const std::string& name) {
            ...
            if (name == "is") {
                return make_builtin(name, Kind::Compare, 2, {"left", "right"}, "Forms an identity or classification relation.",
                    [](Runtime& rt, const std::vector<std::string>& args) {
                        ...
                        if ((left.is_bool() || left.is_int() || left.is_double()) && (right.is_bool() || right.is_int() || right.is_double())) {
                            rt.stack_.push_back(left.as_double() == right.as_double());
                        } else {
                            rt.stack_.push_back(left.as_string() == right.as_string());
                        }
                    });
            }
            if (name == "to") {
        ...
        void create_custom_token_interactive(const std::string& forced_name = {}) {
            ...
            if (is_reserved_word(tok.name)) throw std::runtime_error("reserved structural word cannot be user-defined: " + tok.name);
            ...
            if (definition_is_recursive(tok)) {
                throw std::runtime_error("recursive definition rejected for token: " + tok.name);
            }
        ...
        void modify_token_interactive(const std::string& name) {
            if (is_reserved_word(name)) throw std::runtime_error("reserved structural word cannot be modified: " + name);
        ...
        void execute_tokens(const std::vector<std::string>& toks, const std::string& source_name) {
            ...
            auto is_phrase_break = [&](const std::string& s) -> bool {
                return is_phrase_boundary_word(s) || is_keytoken(s);
            };
            ...
            auto collect_side = [&](std::size_t i, int dir, const std::vector<bool>& consumed) {
                ...
                if (dir < 0) {
                    for (std::size_t j = i; j > 0;) {
                        --j;
                        if (consumed[j] || is_phrase_break(toks[j])) break;
                        idx.push_back(j);
                    }
            ...
            auto join_indices = [&](const std::vector<std::size_t>& indices) -> std::string {
                std::vector<std::string> pieces;
                pieces.reserve(indices.size());
                for (std::size_t idx : indices) pieces.push_back(toks[idx]);
                return join_tokens(pieces);
            };
            ...
            for (std::size_t i = 0; i < toks.size(); ++i) {
                ...
                if (call.builtin && is_determiner_word(call.token)) {
                    const auto phrase = collect_side(i, +1, consumed);
                    if (!phrase.empty()) {
                        for (std::size_t idx : phrase) consumed[idx] = true;
                        call.args.push_back(join_indices(phrase));
                        calls.push_back(std::move(call));
                        continue;
                    }
                }

                if (call.builtin && is_relation_word(call.token)) {
                    const auto left = collect_side(i, -1, consumed);
                    const auto right = collect_side(i, +1, consumed);
                    if (!left.empty() || !right.empty()) {
                        for (std::size_t idx : left) consumed[idx] = true;
                        for (std::size_t idx : right) consumed[idx] = true;
                        if (!left.empty()) call.args.push_back(join_indices(left));
                        if (!right.empty()) call.args.push_back(join_indices(right));
                        calls.push_back(std::move(call));
                        continue;
                    }
                }
        ...
        void execute_token(const std::string& tok, const std::vector<std::string>& args) {
            if (auto it = custom_.find(tok); it != custom_.end()) {
                const auto inserted = active_custom_tokens_.insert(tok);
                if (!inserted.second) {
                    throw std::runtime_error("recursive definition detected: " + tok);
                }
                try {
                    expand_and_execute_custom(it->second, args);
                } catch (...) {
                    active_custom_tokens_.erase(inserted.first);
                    throw;
                }
                active_custom_tokens_.erase(inserted.first);
                return;
        ...
        bool prompt_make_or_skip(const std::string& tok, const std::string& source_name) {
            if (is_reserved_word(tok)) {
                std::cout << "Reserved structural word '" << tok << "' in " << source_name << " cannot be defined.\n";
                return true;
            }
            ...
            while (std::getline(in, line)) {
                ...
                if (line == "===END===") {
                    if (!current.name.empty()) {
                        ...
                        if (is_reserved_word(current.name)) {
                            std::cout << "Skipping reserved structural word in database: " << current.name << "\n";
                        } else if (definition_is_recursive(current)) {
                            std::cout << "Skipping recursive custom token in database: " << current.name << "\n";
                        } else {
